...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:#想要偵測得更具體可以提高信度
    ...
    while cap.isOpened():
        ...
        try:
                ...
                shoulder_L = [landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].x,landmarks[mp_pose.PoseLandmark.LEFT_SHOULDER.value].y]  # landmarks獲取座標(取左肩X值，取Y值)
                elbow_L = [landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].x,landmarks[mp_pose.PoseLandmark.LEFT_ELBOW.value].y]
                wrist_L = [landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].x,landmarks[mp_pose.PoseLandmark.LEFT_WRIST.value].y]
                #get coordinates:right
                shoulder_R = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y]  # landmarks獲取座標(取左肩X值，取Y值)
                elbow_R = [landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_ELBOW.value].y]
                wrist_R = [landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_WRIST.value].y]

                #calculate angle
                angle_L = calculate_angle(elbow_L,shoulder_L, wrist_L)
                angle_R = calculate_angle(elbow_R, shoulder_R, wrist_R)

                #visualize視覺化 angle
                # cv2.putText(image, str(angle_L), tuple(np.multiply(elbow_L, [640, 480]).astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 5, cv2.LINE_AA)#用中間點*頁面大小
                # cv2.putText(image, str(angle_R), tuple(np.multiply(elbow_R, [640, 480]).astype(int)),cv2.FONT_HERSHEY_SIMPLEX, 3, (255, 255, 255), 5, cv2.LINE_AA)  # 用中間點*頁面大小

                #counter:raise arm time
                if angle_R > 130 and angle_L < 50:
                    stage = "down"
                if angle_R < 100 and angle_L > 80 and stage == 'down':#希望做動作時是由down的狀態往up的狀態，不希望手直接舉高
                    stage = "up"
                    counter += 1
                    print(counter)
                    if counter == 10:
