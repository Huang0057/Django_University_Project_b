...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:#想要偵測得更具體可以提高信度
    ...
    cv2.setWindowProperty('MediaPipe Pose', cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)  # 将窗口设置为全屏
    # 打开摄像头
    ...
    while cap.isOpened():
        ...
        try:
                ...
                #get coordinates
                shoulder = [landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].x, landmarks[mp_pose.PoseLandmark.RIGHT_SHOULDER.value].y]  # landmarks獲取座標(取左肩X值，取Y值)
                ...
                if angle < 105:
                    stage = "down"
                if angle > 100 and stage == 'down':#希望做動作時是由down的狀態往up的狀態，不希望手直接舉高
                    stage = "up"
                    counter += 1
                    print(counter)
                    if counter == 10:
